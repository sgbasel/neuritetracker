...
//================================================================
{
    ...
    for(k = 0; k < number_of_points; k++)
    {
        ...
        bool neighbor_of_an_existing_branch = false;
        while(U[point] > 0 && !neighbor_of_an_existing_branch)
        {
            for (j = 0; j < connectivity_large; j++)
            {
                npoint=point+NeighborhoodLarge[j];
                if ( (Filaments[npoint] > 0) && (Filaments[npoint] != k+1) )
                {
                    neighbor_of_an_existing_branch = true;
                    continue;
                }
            }
            if(!neighbor_of_an_existing_branch)
            {
                Umin = U[point];
                npoint_Umin = point;
...
//================================================================                    
{
	...
	Filaments = (unsigned int*) mxGetPr(plhs[0]);
	//==================================================================
